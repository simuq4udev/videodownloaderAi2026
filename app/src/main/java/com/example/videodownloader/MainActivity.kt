...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        lifecycleScope.launch {
            settingsStore.settingsFlow.collect { appSettings = it }
        }
    ...
    private fun startDownload(url: String) {
        val active = adapter.activeDownloadIds().size
        if (active >= appSettings.maxSimultaneousDownloads) {
            Toast.makeText(
                this,
                getString(R.string.max_simultaneous_reached, appSettings.maxSimultaneousDownloads),
                Toast.LENGTH_SHORT
            ).show()
            return
        }
        ...
        request.setDestinationInExternalPublicDir(destination.first, destination.second)
    ...
    private fun resolveDestination(fileName: String): Pair<String, String> {
        val storedUri = appSettings.downloadDirectoryUri ?: return Environment.DIRECTORY_MOVIES to "VideoDownloader/$fileName"
        return try {
            val docId = DocumentsContract.getTreeDocumentId(Uri.parse(storedUri))
            if (!docId.startsWith("primary:")) {
                Environment.DIRECTORY_MOVIES to "VideoDownloader/$fileName"
            } else {
                val relative = docId.removePrefix("primary:").trim('/').ifBlank { "Movies/VideoDownloader" }
                val segments = relative.split('/').filter { it.isNotBlank() }
                val dirType = segments.firstOrNull() ?: Environment.DIRECTORY_MOVIES
                val rest = segments.drop(1).joinToString("/")
                val subPath = if (rest.isBlank()) fileName else "$rest/$fileName"
                dirType to subPath
            }
        } catch (_: Exception) {
            Environment.DIRECTORY_MOVIES to "VideoDownloader/$fileName"
