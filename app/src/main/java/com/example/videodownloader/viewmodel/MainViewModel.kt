        ...
        viewModelScope.launch {
            try {
                ...
                if (directUrl.isNullOrBlank()) {
                    replaceItem(item.copy(status = DownloadStatus.ERROR, errorMessage = "Could not find a public direct video URL. Open the post in browser/app first, then retry with a public post link."))
                    return@launch
                }
